...
function Logger () {
  ...
  try {
    // From everywhere in the application require.main.filename is the original node script path.
    const moduleConfigFile = path.resolve(path.dirname(require.main.filename), 'module.yml')
    const module = Yaml.load(moduleConfigFile)
    this.moduleName = module.name
  } catch (error) {
    throw new Error('Logger couldn\'t find module.yml in root directory of node application.')
  }
