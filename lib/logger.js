...
function Logger () {
  ...
  const module = Yaml.load(path.resolve(__dirname, 'module.yml'))
...
Logger.prototype.log = function (level, message) {
  ...
  const formattedLog = this.formatLog(level, this.moduleName, new Date(Date.now()), message)
  console.log(JSON.stringify(formattedLog) + '\n')
}

Logger.prototype.formatLog = function (level, moduleName, timestamp, message) {
